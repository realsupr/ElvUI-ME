...
function Style(self, totem, flyout)
	...
	if Count then
		...
		Count:SetPoint("BOTTOMRIGHT", 0, E.Scale(1))
	...
	if Btname then
		Btname:SetFont(C["media"].font, 9, "OVERLAY")
	...
	if HotKey then
		...
		HotKey:SetPoint("TOPRIGHT", 1, E.Scale(-2))
		HotKey:SetFont(C["media"].font, 10, "THINOUTLINE")
